import type { BoneState, JointDef, Vec3 } from "@/lib/game/sim/types";
...
export function applyJointConstraint(
  ...
  const linear =
    err * joint.linearStiffness * stiffnessScale + relVel * joint.linearDamping;
  ...
  const ax = (desiredX - child.x) * joint.angularStiffness * stiffnessScale - child.vx * joint.angularDamping;
  const ay = (desiredY - child.y) * joint.angularStiffness * stiffnessScale - child.vy * joint.angularDamping;
  const az = (desiredZ - child.z) * joint.angularStiffness * stiffnessScale - child.vz * joint.angularDamping;
  child.vx += ax * invC;
  child.vy += ay * invC;
  child.vz += az * invC;
  ...
  if (swing > joint.swingLimit) {
    const pull = (swing - joint.swingLimit) * joint.angularStiffness * stiffnessScale;
